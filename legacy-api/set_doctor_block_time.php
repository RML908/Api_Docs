...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $token = $headers['Authorization'];
 	...
 	$block_start = isset($_POST['block_start']) ? $_POST['block_start'] : '';
 	$block_end = isset($_POST['block_end']) ? $_POST['block_end'] : '';
 	$note = isset($_POST['note']) ? $_POST['note'] : '';

  if (empty($doctor_ssn) || empty($block_start) || empty($block_end)) {
    $check_key = array();
    $temp = array();
    $temp['status'] = '0';
