...
router.post('/', authenticate, requireAdmin, validateBody(CreateApiKeySchema), async (req, res, next) => {
  try {
...
router.patch('/:id/revoke', authenticate, requireAdmin, async (req, res, next) => {
  try {
