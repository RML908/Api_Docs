...
router.get('/', publicCache(30), async (_req, res, next) => {
  try {
...
router.post('/', authenticate, requireAdmin, writeRateLimiter, validateBody(CreateGroupSchema), async (req, res, next) => {
  try {
...
router.patch('/:id', authenticate, requireAdmin, writeRateLimiter, validateBody(UpdateGroupSchema), async (req, res, next) => {
  try {
